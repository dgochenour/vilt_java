...
/**
* A sequence of ShapeTypeExtended instances.
*/
public final class ShapeTypeExtendedSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShapeTypeExtendedSeq() {
        super(ShapeTypeExtended.class);
    }

    public ShapeTypeExtendedSeq (int initialMaximum) {
        super(ShapeTypeExtended.class, initialMaximum);
    }

    public ShapeTypeExtendedSeq (Collection<?> elements) {
        super(ShapeTypeExtended.class, elements);
    }

    public  ShapeTypeExtended get(int index) { 
        return (ShapeTypeExtended) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShapeTypeExtended.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShapeTypeExtended objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShapeTypeExtended.create());
